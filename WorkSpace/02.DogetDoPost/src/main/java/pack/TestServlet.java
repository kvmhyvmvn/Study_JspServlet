...
public class TestServlet extends HttpServlet {
	...
	// URL : 톰캣 (localhost:portNumber(192.168.0.31:8080)/ Web Project / Mapping)
	// 192.168.0.31:8080/do/te Get방식 요청 시 doGetMethod가 자동으로 실행됨(TomCat)
	// 프로젝트를 새로 만들었을때에는 Tomcat이 프로젝트를 인식할 수 있게 add & remove를 통해서 프로젝트를 넣어준다.

	// HttpServletRequest : Request객체 톰캣에서 요청이 오면 모든 정보를 Request를 통해 담아서 Java코드로 준다.
	// 요청에 관련 된 모든 기능을 담당한다.
	// HttpServletResponse : Response객체 Request 객체로 요청 처리를 받고 Client(크롬)에 응답을 주기 위한 객체
	// 페이지 전환, 페이지에 내용 표시 등의 크롬창의 표시를 담당함
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		System.out.println("요청 되면 실행");
		String param = req.getParameter("param");
		System.out.println("크롬을 통해 GET방식으로 param 정보가 들어옴" + param);

		// req, res
		// 요청, 응답
		// 요청을 받았을 때 새로운 페이지를 요청한다.(req)
